...
from msf.core import printer
from msf.lib.file import list_dir
from msf.lib.dbconnect import DBConnect
...
class Framework():
    ...
    def __init__(self):
        ...
        loadable_modules = [
            m for m in list_dir(self.modules_path) 
            if m.endswith('.py') and not m.startswith('__') and m.lower() == m
        ]
    ...
    def set_module(self, module_name, module):
        ...
        if not isinstance(module_instance, Templates):
            self.print_error('%s is not child of Templates class' % module_name)
            return
    ...
    def run(self, args, module_name=None):
        ...
        missing_options = self.options.get_missing_options() + module.options.get_missing_options()
